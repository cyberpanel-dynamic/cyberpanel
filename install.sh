...
check_valid_os() {
    ...
    echo "\nDetecting CentOS 7...\n"
		...
		yum update -y
		yum install curl wget git jq iputils -y 1> /dev/null
		yum update curl wget git jq iputils ca-certificates -y 1> /dev/null
                SERVER_OS="CentOS"
    ...
    SERVER_OS="AlmaLinux"
		...
		apt update -y
		apt install -y -qq wget curl git jq iputils-ping
    SERVER_OS="Ubuntu"
