...
sed -i "s|git clone $GIT_REPO|echo downloaded|g' install.py
...
#line1="$(grep -n "$GIT_REPO" install.py | head -n 1 | cut -d: -f1)"
#line2=$((line1 - 1))
...
sed -i 's|git clone '$GIT_REPO'|wget https://cyberpanel.sh/cyberpanel-git.tar.gz \&\& tar xzvf cyberpanel-git.tar.gz|g' install.py
...
wget -O requirements.txt '$RAW_GIT_REPO'/1.8.0/requirments.txt
...
if [[ $DEV == "ON" ]] ; then
	#install dev branch 
	#wget '$RAW_GIT_REPO'/$BRANCH_NAME/requirments.txt
	cd /usr/local/
	python3.6 -m venv CyberPanel
	source /usr/local/CyberPanel/bin/activate
	wget -O requirements.txt '$RAW_GIT_REPO'/$BRANCH_NAME/requirments.txt
